...
function drawMessageList(messageList){
    ...
    for (i = 0; i < messageList.length; i++){
        ...
        var notlikeCount = updateRateIconsAndCounts(textareaText, messageList[i]);
...
//Обработка события нажатия на кнопку "Отправить сообщение" при отправке нового сообщения
$('#sendButton').click(function() {
    var error = false;
    $(".inputField").each(function(){
        var text = $.trim($(this).val());
        if(text == "" || text == "Введите значение"){
            error=true;
        }
    })
    if(error == true){
...
function insertNewMessage(message){
    ...
    var notlikeCount = updateRateIconsAndCounts(textareaText, message);
...
function updateMessageRate(message){
    ...
    updateRateIconsAndCounts(divTextOfMessage, message);

}

//Функция обновления иконок кнопок рейтинга и количества
function updateRateIconsAndCounts(textareaText, message){
    var like;
    var likeCount;
    var notlike;
    var notlikeCount;
